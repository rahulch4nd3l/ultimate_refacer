...
def run(video, *vars):
    ...
    origins = vars[0:num_faces]
    destinations = vars[num_faces:num_faces * 2]
    thresholds = vars[num_faces * 2:]
...
with gr.Blocks() as demo:
    with gr.Row():
        gr.Markdown("# Refacer")
    with gr.Row():
        video = gr.File(label="Upload Video")  # Use gr.File for video uploads
